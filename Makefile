# HyperEngine .a Makefile
# Pls leeme
# Este makefile está retocado del makefile de compilar imgui en un .a
# No es perfecto pues si quito los main.cpp y main.hpp ya no encuentra el resto de archivos en las subcarpetas
# El modo de usarlo es ir cambiando entre este makefile y el principal, cambiando de nombre a Makefile


###################################
# Hyper Adrenaline .a    Makefile #
###################################
#---------------------------------#
# USAGE 						  #
#---------------------------------#
...
OBJ			:= build-a
## Found files
ALLCPPS		:= $(shell find $(SRC)/ -type f -iname *.cpp | grep -v $(SRC)/main.cpp)
...
IMGUIOBJSUBDIR := $(LIB)/imgui/build
ALLIMGUIOBJ := $(shell find $(LIB)/imgui/build/ -type f -iname *.o)
...
$(APP) : $(OBJSUBDIRS) $(ALLOBJS) $(IMGUIOBJSUBDIR) $(ALLIMGUIOBJ)
	$(AR) $(ARFLAGS) $(APP) $(ALLOBJS) $(ALLIMGUIOBJ)
	$(RANLIB) $(APP)
